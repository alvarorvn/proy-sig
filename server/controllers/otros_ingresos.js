...
async function addOtroIngreso(req, res) {
    var { ingr_abono, ingr_descripcion, mes_id, anio_id } = req.body;
    if (ingr_abono === null) ingr_abono = '';
    ...
    try {
        ...
        sql = `INSERT INTO otros_ingresos 
            ...
            VALUES (${ingr_abono}, '${ingr_descripcion.toUpperCase()}', '${mes_id}', '${anio_id}')`;
...
async function getAllOtrosIngresos(req, res) {
    try {
        ...
        if (result.length == 0) return res.json({ message: "No hay otros ingresos registrados", result });
        result.forEach(oi => {
            let obj = {};

            obj.ingr_id = oi[0]; obj.ingr_abono = oi[1]; obj.ingr_descripcion = oi[2];
            obj.mes_nombre = oi[3]; obj.mes_id = oi[4]; obj.anio_numero = oi[5]; obj.anio_id = oi[6];

            otros_ingresos.push(obj);
        });
        return res.json(otros_ingresos);
    } catch (error) {
        return res.json({ message: "Error al obtener cobros" });
    }
}
